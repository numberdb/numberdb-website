...
This file is vendored into the client package as
``clients/python/numberdb/_polynomial.py``, byte for byte, and a test asserts
they are identical. It defines the polynomial search key, and both sides of the
network must produce the same bytes: a lookup sends a digest of the key rather
than the polynomial itself, because the longest stored polynomial is 58866
characters and a URL is rejected past 8k. Change this file, then copy it across.
...
except ImportError:  # pragma: no cover - vendored into the client package
    class ParseError(ValueError):  # type: ignore[no-redef]
        """Text that cannot be read as the number it claims to be.

        Defined here when the sibling module is absent, which is the case in
        the client package: it carries this file alone, since the polynomial
        key is the only thing both sides must compute identically.
        """
