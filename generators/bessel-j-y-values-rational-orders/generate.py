"""Values of Bessel functions J_nu and Y_nu -- numberdb.org/T187

The principal real values J_nu(x) and Y_nu(x), for rational order nu and
...
class BesselJYValues(numberdb.Generator):

    table = os.environ.get("NUMBERDB_TABLE", "T187")
    parameters = ("function", "nu", "x")
    type = "R"
    digits = 100
    rigour = "proven"
    ...
    def enumerate(self, orders=ORDERS, denominator=MAX_DENOMINATOR,
        ...
        for function in ("J", "Y"):
            for nu in orders:
                for x in arguments:
                    yield {"function": function, "nu": nu, "x": x}
    ...
    def value(self, params, digits):
        ...
        comment = _comment(function, nu_text)
        if comment:
...
if __name__ == "__main__":
    _key_from_stdin()
    generator = BesselJYValues()
    if os.environ.get("NUMBERDB_PUBLISH") == "1" or "--publish" in sys.argv:
        print(generator.publish(
            message="Bessel J and Y values at rational orders and arguments"))
    else:
