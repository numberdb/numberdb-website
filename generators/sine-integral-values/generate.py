"""Values of the sine integral -- numberdb.org/T192

The real values of Si(x) at
...
#
# So: every argument of two decimal places up to 10. Si is wanted across its first oscillations, where it climbs past
# its limit pi/2 and settles back; ten covers three of them.
STEP = QQ(1) / QQ(100)
MAX_ARGUMENT = 10
...
class SineIntegralValues(numberdb.Generator):

    table = os.environ.get("NUMBERDB_TABLE") or "T192"
    parameters = ("x",)
    type = "R"
    digits = 100
    rigour = "proven"
...
if __name__ == "__main__":
    _key_from_stdin()
    generator = SineIntegralValues()
    if "--publish" in sys.argv or os.environ.get("NUMBERDB_PUBLISH") == "1":
        print(generator.publish(
            message="values of the sine integral"))
    else:
