"""Hirzebruch L-polynomials -- numberdb.org/T336.

This generator stores the homogeneous Hirzebruch L-polynomials

    L_n(p1, p2, ...) = [degree n] prod_i sqrt(x_i) / tanh(sqrt(x_i)),

where pj is the j-th elementary symmetric polynomial in the Pontryagin roots
x_i. The table starts at n = 1 and stops at n = 6, because L_7 has a nonzero
p7 term and the database searches polynomials in at most six variables.
...
first three nonconstant components with standard printed formulas, verify the
Pontryagin-root definition in seven roots, and check that substituting the
Pontryagin classes of T CP^(2m) gives signature 1 for 1 <= m <= 6.
"""
...
TABLE = os.environ.get("NUMBERDB_TABLE", "T336")
MAX_DEGREE = 6
CHECK_DEGREE = 7
...
def l_series_coefficients(degree):
    """Coefficients of sqrt(x) / tanh(sqrt(x)) through x^degree."""
    return [
        (QQ(2) ** (2 * k)) * bernoulli_plus(2 * k) / factorial(2 * k)
...
def total_l_polynomial():
    log_q = log_series(l_series_coefficients(CHECK_DEGREE), CHECK_DEGREE)
...
def l_polynomial(n):
    return to_output_ring(homogeneous_weighted(TOTAL_L, int(n)))


def root_q(root):
    coeffs = l_series_coefficients(CHECK_DEGREE)
...
def complex_projective_space_substitution(polynomial, m):
    """Substitute p_j(T CP^(2m)) = binomial(2m + 1, j) h^j."""
    images = [
        binomial(2 * int(m) + 1, j) * H ** j
...
def check_identities():
    expected = {
        1: OUTPUT_P[0] / QQ(3),
        2: (7 * OUTPUT_P[1] - OUTPUT_P[0] ** 2) / QQ(45),
        3: (
            62 * OUTPUT_P[2]
            - 13 * OUTPUT_P[0] * OUTPUT_P[1]
            + 2 * OUTPUT_P[0] ** 3
        ) / QQ(945),
    }
    for n, value in expected.items():
        found = l_polynomial(n)
        if found != value:
            raise AssertionError("L_%s printed formula failed: %s != %s"
                                 % (n, found, value))

    l7 = homogeneous_weighted(TOTAL_L, 7)
    p7_coefficient = l7.monomial_coefficient(CHECK_P[6])
    if p7_coefficient == 0:
        raise AssertionError("L_7 unexpectedly has no p7 term")

    via_roots = root_product_l()
    via_components = substitute_check_to_roots(TOTAL_L)
    if via_components != via_roots:
        raise AssertionError("root-product check failed")

    for m in range(1, MAX_DEGREE + 1):
        value = complex_projective_space_substitution(l_polynomial(m), m)
        if value[m] != 1:
            raise AssertionError(
                "signature of CP^(2*%s) failed: coefficient is %s"
                % (m, value[m])
            )

    lengths = [
        (len(str(l_polynomial(n))), n)
        for n in range(1, MAX_DEGREE + 1)
    ]
    longest = max(lengths, key=lambda item: item[0])
    print("integrity checks passed for %d Hirzebruch L-polynomials"
          % MAX_DEGREE)
    print("matched the printed formulas for L_1 through L_3")
    print("root-product and projective-space signature checks passed")
    print("L_7 has p7 coefficient %s" % p7_coefficient)
    print("longest polynomial has %d characters at n=%s"
          % (longest[0], longest[1]))


class HirzebruchLPolynomials(numberdb.Generator):
    ...
    def value(self, params, digits):
        return l_polynomial(ZZ(params["n"]))
...
def check_stored_values():
    ...
    for n in range(1, MAX_DEGREE + 1):
        expected = l_polynomial(n)
        if values.get(ZZ(n)) != expected:
            raise AssertionError("stored L_%s failed: %s != %s"
...
def main():
    _key_from_stdin()
    check_identities()
    generator = HirzebruchLPolynomials()
    if os.environ.get("NUMBERDB_PUBLISH") == "1" or "--publish" in sys.argv:
        print(fill_draft_once(
            generator,
            message="fill Hirzebruch L-polynomials from exact characteristic series",
        ))
    else:
